...
EGIT_SUBMODULES=( '-*' 'dynarmic' 'sirit' 'xbyak' 'tzdb_to_nx'
	              'externals/nx_tzdb/tzdb_to_nx/externals/tz/tz' 'VulkanMemoryAllocator' 'Vulkan-Utility-Libraries' 'simpleini')
...
SLOT="0"
KEYWORDS=""
IUSE="+cubeb discord +qt5 sdl +system-libfmt +system-vulkan test webengine +webservice"
...
src_prepare() {
	...
	# Unbundle cubeb
	sed -i '/^# Cubeb/,/^endif()/d' externals/CMakeLists.txt || die
	...
	# Unbundle enet
	sed -i -e '/^# ENet/,/^endif()/d' externals/CMakeLists.txt || die
